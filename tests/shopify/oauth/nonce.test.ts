...
describe("Shopify OAuth Nonce Management", () => {
  ...
  const MockNonceManagerValid = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return {
        generate: () => Effect.sync(() => nanoid() as Nonce),
        store: (nonce: Nonce) =>
          Effect.gen(function* () {
            yield* Ref.update(
              storedNonces,
  ...
  describe("Nonce Generation", () => {
    it.scoped("should generate unique nonces", () =>
      Effect.gen(function* () {
        ...
        // Test that nonces have the correct format (nanoid)
        const nanoidRegex = /^[A-Za-z0-9_-]{21}$/; // NanoID default is 21 characters
        expect(nanoidRegex.test(nonce1)).toBe(true);
        expect(nanoidRegex.test(nonce2)).toBe(true);
