...
export const ConversationMCPServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const aiIntentService = yield* AIIntentService;
    ...
    return {
      processMessage: (
        ...
        return Effect.gen(function* () {
          ...
          const intent = yield* aiIntentService.classifyIntent(
            message,
            context
          );
      ...
      manageCart: (conversationId: ConversationId, action: CartAction) =>
        Effect.gen(function* () {
          switch (action.type) {
            case "get": {
              const cart = yield* mcpService.getCart(action.cartId);
              if (!cart) {
                return yield* mcpService.getCart(); // Get default cart
              }
              return cart;
            }
