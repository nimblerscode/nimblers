...
export async function acceptInvitationAction(
  ...
  const program = Effect.withSpan("accept-invitation-action", {
    attributes: {
      "action.type": "accept-invitation",
      "request.method": request.method,
      "request.content_type": request.headers.get("content-type") || "unknown",
    },
  })(
    Effect.gen(function* () {
      ...
      const tokenPayload = yield* Effect.withSpan("verify-invitation-token", {
        attributes: {
          "invitation.token_length": token.length,
          "user.id": userId,
        },
      })(
        pipe(
      ...
      const invitation = yield* Effect.withSpan("get-invitation", {
        attributes: {
          "invitation.organization_slug": organizationSlug,
          "invitation.token": token.substring(0, 8) + "...", // Only log first 8 chars for security
        },
      })(
        pipe(
          InvitationDOService,
      ...
      const acceptanceResult = yield* Effect.withSpan(
