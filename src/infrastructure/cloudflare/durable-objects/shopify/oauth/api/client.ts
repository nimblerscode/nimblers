...
import { shopifyOAuthApi } from "./handlers";

/**
 * Shopify OAuth DO Client using HttpApiClient.make()
 *
 ...
 * from the shopifyOAuthApi definition in handlers.ts!
 */

/**
 * Creates a Shopify OAuth DO client using HttpApiClient.make()
 *
 ...
 * Usage:
 * ```typescript
 * const doId = shopifyOAuthDONamespace.idFromName(shopId);
 * const stub = shopifyOAuthDONamespace.get(doId);
 * const client = yield* createShopifyOAuthDOClient(stub);
 *
 * // Auto-generated methods with perfect type safety!
 * const nonce = yield* client.shopifyOAuth.generateNonce();
 *
 * const tokenResult = yield* client.shopifyOAuth.storeAccessToken({
 *   payload: {
 ...
 * ```
 */
export const createShopifyOAuthDOClient = (stub: DurableObjectStub) =>
  Effect.gen(function* () {
    ...
    return yield* HttpApiClient.make(shopifyOAuthApi, {
